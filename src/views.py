...
def encrypt(request):
    if request.method == 'POST':
        ...
        unique_cover_file_name = os.path.join(
            destination_directory,
            'cover_' + video_cover_file.name
        )

        unique_hide_file_name = os.path.join(
            destination_directory,
            'hide_' + video_hide_file.name
        )
        ...
        video_name = encode(
            unique_cover_file_name,
            unique_hide_file_name
        )
...
def enroll(file):
    ...
    try:
        ...
        print(
            "Failed to load weights from the weights file, "
            "please ensure *.pb file is present in the MODEL_FILE directory"
        )
        exit()
    ...
    np.save(
        os.path.join(p.EMBED_LIST_FILE, speaker + ".npy"),
        enroll_embs
    )
...
def encode(video_cover, video_hide):
    ...
    container_outvid = cv2.VideoWriter(
        'App/media/results/{}.avi'.format(name),
    ...
    container = cv2.VideoWriter(
        'App/media/results/{}1.avi'.format(name),
    ...
    num_frames = int(
        vidcap1.get(cv2.CAP_PROP_FRAME_COUNT)
    )
    ...
    while True:
        ...
        secret = cv2.resize(
            cv2.cvtColor(secret, cv2.COLOR_BGR2RGB),
            (224, 224),
            interpolation=cv2.INTER_AREA
        )

        cover = cv2.resize(
            cv2.cvtColor(cover, cv2.COLOR_BGR2RGB),
            (224, 224),
            interpolation=cv2.INTER_AREA
        )
        ...
        if frame % 4 == 0:
            ...
            coverout = model.predict(
                [
                    normalize_batch(secret_batch),
                    normalize_batch(cover_batch)
                ]
            )
...
def decrypt(request):
    if request.method == 'POST':
        ...
        video_user = AuthDetails.objects.filter(
            video_name=video_name,
            audio=enroll_name,
            password=password
        )
        ...
        if video_user.exists():
            ...
            return render(
                ...
                'decrypt.html',
                {
                    'video_name': '{}secret.avi'.format(video_name),
                    'name': name
                }
            )
...
def recognize(file):
    if os.path.exists(p.EMBED_LIST_FILE):
        ...
        print(
            "Loading model weights from [{}]....".format(
                p.MODEL_FILE
            )
        )
        ...
        if min(list(distances.values())) < p.THRESHOLD:
            ...
            print(
                ...
                "try enrolling again with a clear voice sample"
            )
...
def reveal_video(video, name):
    ...
    print(
        "Total frames in container video:",
        num_frames
    )

    secret_outvid = cv2.VideoWriter(
        'App/media/results/{}secret.avi'.format(name),
    ...
    while True:
        ...
        if frame % 4 == 0:
            cover_batch = (
                np.float32(cover_batch) / 255.0
            )

            secretout = model1.predict(
                [normalize_batch(cover_batch)]
            )

            secretout = denormalize_batch(
                secretout
            )

            secretout = (
                np.squeeze(secretout) * 255.0
            )
